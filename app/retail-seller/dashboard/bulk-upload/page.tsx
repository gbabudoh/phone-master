...
export default function BulkUploadPage() {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    // Simulate upload - in real app, this would call an API
    setTimeout(() => {
      setResult({
        success: 45,
        failed: 3,
        errors: [
          'Row 12: Invalid IMEI format',
          'Row 28: Missing required field "price"',
          'Row 41: Duplicate product SKU',
        ],
      });
      setUploading(false);
    }, 2000);
  ...
  const handleDownloadCSV = () => {
    ...
    link.download = 'retail-products-template.csv';
...
"EE Top Up £20","EE network top up voucher worth £20",20.00,50,service_voucher,,,,,,,,,,,,,top_up,EE,30 days,EE`;
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = 'retail-products-template.xlsx';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="mt-2 text-foreground/60">Upload multiple products at once using CSV or Excel files</p>
      </div>
      ...
      <div className="mb-6 rounded-lg border border-accent-grey/20 bg-white p-6">
        ...
        <p className="mb-4 text-sm text-foreground/60">
          Download our template file and fill in your product details. Make sure to follow the format guidelines.
        </p>
        <div className="flex flex-wrap gap-3">
          <button onClick={handleDownloadCSV} className="flex items-center space-x-2 rounded-lg border border-primary bg-white px-4 py-2 text-sm font-medium text-primary transition-colors hover:bg-accent-cyan-light cursor-pointer">
            <Download className="h-4 w-4" />
            <span>Download CSV Template</span>
          </button>
          <button onClick={handleDownloadExcel} className="flex items-center space-x-2 rounded-lg border border-primary bg-white px-4 py-2 text-sm font-medium text-primary transition-colors hover:bg-accent-cyan-light cursor-pointer">
            <Download className="h-4 w-4" />
      ...
      <div className="mb-6 rounded-lg border border-accent-grey/20 bg-white p-6">
        ...
        <div className="mb-4">
          <label
            htmlFor="file-upload"
            className="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-accent-grey/30 bg-accent-cyan-light/30 p-8 transition-colors hover:border-primary hover:bg-accent-cyan-light"
          >
            <FileSpreadsheet className="mb-3 h-12 w-12 text-primary" />
            <span className="text-sm font-medium text-foreground">
              {file ? file.name : 'Click to upload or drag and drop'}
            </span>
            <span className="mt-1 text-xs text-foreground/60">CSV or Excel files (max 10MB)</span>
            <input
              id="file-upload"
              type="file"
              accept=".csv,.xlsx,.xls"
        ...
        <div className="rounded-lg border border-accent-grey/20 bg-white p-6">
          ...
          <div className="mb-4 grid grid-cols-2 gap-4">
            <div className="rounded-lg bg-green-50 p-4">
              ...
              <p className="mt-2 text-2xl font-bold text-green-700">{result.success}</p>
            </div>
            <div className="rounded-lg bg-red-50 p-4">
              ...
              <p className="mt-2 text-2xl font-bold text-red-700">{result.failed}</p>
            </div>
          </div>

          {result.errors.length > 0 && (
            <div>
              <h3 className="mb-2 text-sm font-semibold text-foreground">Errors:</h3>
      ...
      <div className="mt-6 rounded-lg border border-accent-grey/20 bg-white p-6">
        ...
        <ul className="space-y-2 text-sm text-foreground/70">
          ...
          <li>• Images should be valid URLs (up to 5 per product)</li>
