...
import { Package, TrendingUp, PoundSterling, Eye, Plus, Upload, ShoppingCart, Truck, ArrowUpRight, ArrowDownRight, Globe, Users } from 'lucide-react';
    ...
    <div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-foreground">
              {user ? `Welcome back, ${getUserDisplayName()}` : 'Wholesale Dashboard'}
            </h1>
            {getStoreName() && (
              <p className="mt-1 text-lg font-semibold text-purple-600">{getStoreName()}</p>
            )}
            <p className="mt-1 text-foreground/60">
              Manage your wholesale operations and bulk orders
            </p>
          </div>
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-xl border border-accent-grey/20 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-foreground/60">Bulk Orders</p>
              <p className="mt-1 text-2xl font-bold text-foreground">{stats.bulkOrders || 0}</p>
              <div className="mt-1 flex items-center text-xs text-green-600">
                <ArrowUpRight className="h-3 w-3" />
        ...
        <div className="rounded-xl border border-accent-grey/20 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-foreground/50">Unlimited • Bulk pricing</p>
            </div>
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-4">
        ...
        <div className="rounded-lg border border-accent-grey/20 bg-gradient-to-r from-purple-50 to-purple-100 p-4">
          <p className="text-sm font-medium text-foreground/60">Total Sales</p>
          <p className="mt-1 text-xl font-bold text-purple-700">{stats.totalSales || 0}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="rounded-lg border border-accent-grey/20 bg-white p-6">
          ...
          <div className="space-y-3">
            ...
            <Link
              href="/wholesale-seller/dashboard/bulk-upload"
            ...
            <Link
              href="/wholesale-seller/dashboard/bulk-orders"
              className="flex items-center space-x-2 rounded-lg border border-accent-grey/30 bg-white px-4 py-3 text-foreground transition-colors hover:bg-accent-cyan-light cursor-pointer"
            >
              <Truck className="h-4 w-4" />
              <span>Manage Bulk Orders</span>
            </Link>
            <Link
              href="/wholesale-seller/dashboard/reports"
        ...
        <div className="rounded-lg border border-accent-grey/20 bg-white p-6">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link href="/wholesale-seller/dashboard/orders" className="text-sm text-primary hover:underline cursor-pointer">
              View all
            </Link>
          </div>
      ...
      <div className="mt-8 rounded-xl bg-gradient-to-r from-primary to-primary-dark p-6 text-white">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-bold">Wholesale Seller Benefits</h3>
            <ul className="mt-2 space-y-1 text-sm text-white/80">
              <li>✓ Unlimited product listings</li>
              <li>✓ 5% commission per sale</li>
              <li>✓ Bulk pricing & volume discounts</li>
              <li>✓ Priority B2B support</li>
