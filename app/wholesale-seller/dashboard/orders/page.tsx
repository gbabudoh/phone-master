  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="mt-2 text-foreground/60">View and manage your wholesale orders</p>
      </div>
      {loading ? (<div className="py-12 text-center"><p className="text-foreground/60">Loading...</p></div>
      ) : orders.length > 0 ? (
            ...
            <tbody className="divide-y divide-accent-grey/20 bg-white">
              {orders.map((order) => (
                <tr key={order._id} className="hover:bg-accent-cyan-light/30">
                  ...
                  <td className="px-6 py-4 text-sm text-foreground">{order.items.length} item(s)</td>
                  <td className="whitespace-nowrap px-6 py-4 text-sm text-foreground">{formatPrice(order.totalAmount)}</td>
                  <td className="whitespace-nowrap px-6 py-4"><div className="flex items-center space-x-2">{getStatusIcon(order.payoutStatus)}<span className="text-sm capitalize">{order.payoutStatus}</span></div></td>
