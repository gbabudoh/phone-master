...
export default function NewProductPage() {
  ...
  useEffect(() => {
    if (!authLoading) {
      ...
      if (user.role === 'buyer') { router.push('/personal-seller/dashboard'); return; }
    }
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <Link href="/personal-seller/dashboard/inventory" className="flex items-center space-x-2 rounded-lg border border-accent-grey/20 px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-accent-cyan-light cursor-pointer"><X className="h-4 w-4" /><span>Cancel</span></Link>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="rounded-lg border border-green-200 bg-green-50/30 p-6">
          <div className="flex items-center space-x-2 mb-4"><Settings className="h-5 w-5 text-green-600" /><h2 className="text-lg font-semibold text-green-900">Seller Options</h2></div>
          {/* Discounts */}
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center space-x-2"><Percent className="h-4 w-4 text-green-600" /><label className="text-sm font-medium">Discounts</label></div>
              ...
              <div className="space-y-2">
                {discountTiers.map((tier, index) => (
                  <div key={index} className="flex items-center space-x-3 bg-white rounded-lg p-3 border border-accent-grey/20">
                    <div className="flex-1"><label className="text-xs text-foreground/60">Min Quantity</label><input type="number" min="1" value={tier.minQuantity} onChange={(e) => updateDiscountTier(index, 'minQuantity', e.target.value)} className="w-full rounded border border-accent-grey/20 px-3 py-1.5 text-sm focus:border-primary focus:outline-none" placeholder="e.g., 2" /></div>
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center space-x-2"><Truck className="h-4 w-4 text-green-600" /><label className="text-sm font-medium">Shipping Options</label></div>
            ...
            <div className="space-y-2">
              {shippingOptions.map((option, index) => (
                <div key={index} className="flex items-center space-x-3 bg-white rounded-lg p-3 border border-accent-grey/20">
                  <div className="flex-1"><label className="text-xs text-foreground/60">Name *</label><input type="text" value={option.name} onChange={(e) => updateShippingOption(index, 'name', e.target.value)} className="w-full rounded border border-accent-grey/20 px-3 py-1.5 text-sm focus:border-primary focus:outline-none" placeholder="e.g., Royal Mail" /></div>
          ...
          {/* Terms */}
          <div>
            <div className="flex items-center space-x-2 mb-2"><FileText className="h-4 w-4 text-green-600" /><label className="text-sm font-medium">Terms & Conditions</label></div>
        ...
        <div className="flex items-center justify-end space-x-4">
          <Link href="/personal-seller/dashboard/inventory" className="rounded-lg border border-accent-grey/20 px-6 py-2 text-sm font-medium text-foreground transition-colors hover:bg-accent-cyan-light cursor-pointer">Cancel</Link>
