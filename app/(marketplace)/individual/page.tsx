...
export default function IndividualPage() {
  ...
  const performSearch = async (query: string, filters: FilterState) => {
    ...
    try {
      ...
      params.set('sellerType', 'personal_seller'); // Only personal sellers
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8">
        <h1 className="text-4xl font-bold text-foreground">Individual Marketplace</h1>
        <p className="mt-4 text-lg text-foreground/60">
          Sell your personal devices. Up to 5 listings, simple and straightforward.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-accent-grey/20 bg-white p-12 text-center">
          <p className="text-lg text-foreground/60">No individual listings available at the moment.</p>
        </div>
      )}
    </div>
  );
